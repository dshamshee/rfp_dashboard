    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto p-6 sm:p-8">
        <DialogHeader className="gap-2 border-b pb-4">
          ...
          <DialogTitle className="text-xl font-bold leading-snug text-foreground pt-1">
            {tender.title}
          </DialogTitle>
        ...
        <div className="space-y-6 pt-2">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-muted/30 p-4 rounded-xl border">
              <div>
            ...
            {(tender.expectedMargin != null || tender.expectedMarginRupees != null) && (
              <div className="mt-2 flex items-center gap-4 text-xs bg-emerald-50 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300 p-2.5 rounded-lg border border-emerald-200 dark:border-emerald-900">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div className="p-3 rounded-lg border bg-card">
                ...
                <p className="text-sm font-medium text-foreground">{formatDate(tender.publishDate)}</p>
              </div>
              <div className="p-3 rounded-lg border bg-card">
                <p className="text-xs text-muted-foreground">Pre-Bid Meeting Date</p>
                <p className="text-sm font-medium text-foreground">{formatDate(tender.preBidDate)}</p>
              </div>
              <div className="p-3 rounded-lg border bg-card">
              ...
              <div className="p-3 rounded-lg border bg-card">
                ...
                <p className="text-sm font-medium text-foreground">{formatDate(tender.openingDate)}</p>
              </div>
              <div className="p-3 rounded-lg border bg-card">
                <p className="text-xs text-muted-foreground">Submission Date</p>
                <p className="text-sm font-medium text-foreground">{formatDate(tender.submissionDate)}</p>
              </div>
              <div className="p-3 rounded-lg border bg-card">
                <p className="text-xs text-muted-foreground">Expected Result Date</p>
                <p className="text-sm font-medium text-foreground">{formatDate(tender.expectedResultDate)}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">{tender.partner || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Stage</p>
                <p className="text-sm font-medium text-foreground">{tender.stage || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Technical Status</p>
                <p className="text-sm font-medium text-foreground">{tender.technicalStatus || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Commercial Status</p>
                <p className="text-sm font-medium text-foreground">{tender.commercialStatus || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">EMD Status</p>
                <p className="text-sm font-medium text-foreground">{tender.emdStatus || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Award Status</p>
                <p className="text-sm font-medium text-foreground">{tender.awardStatus || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Competitor</p>
                <p className="text-sm font-medium text-foreground">{tender.competitor || "-"}</p>
              </div>
            </div>
          </div>
          ...
          {(tender.nextAction || tender.remarks) && (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {tender.nextAction && (
                  <div className="p-3.5 rounded-lg border bg-amber-500/5 border-amber-500/20">
